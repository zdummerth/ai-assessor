...
import { getEmployees } from "@/app/admin/employees/queries";
...
const employeeFetcher = async () => {
  const { data, error } = await getEmployees(0, 999);
  if (error) throw error;
  return data || [];
};

export default function EmployeeSelect({ onChange }: EmployeeSelectProps) {
  const id = React.useId();
  const {
    data: employees = [],
    isLoading,
    error,
  } = useSWR("employees", employeeFetcher, { revalidateOnFocus: false });
  ...
  const handleSelectionChange = (value: EmployeeOption | null) => {
    onChange?.(value);
  };

  return (
    <Combobox.Root items={options} onValueChange={handleSelectionChange}>
      <div className="relative flex flex-col gap-1 text-sm leading-5 font-medium text-gray-900">
        <label htmlFor={id}>Choose an employee</label>
        <div className="relative [&>input]:pr-[calc(0.5rem+1.5rem)] has-[.combobox-clear]:[&>input]:pr-[calc(0.5rem+1.5rem*2)]">
          <Combobox.Input
            placeholder={isLoading ? "Loading..." : "e.g. John Doe"}
            id={id}
            disabled={isLoading || !!error}
            className="h-10 w-64 rounded-md font-normal border border-gray-200 pl-3.5 text-base text-gray-900 bg-[canvas] focus:outline focus:outline-2 focus:-outline-offset-1 focus:outline-blue-800 disabled:opacity-50 disabled:cursor-not-allowed"
          />
