...
interface ComboboxNeighborhoodsProps {
  items: BoundaryData[];
  value?: string[];
...
export function ComboboxNeighborhoods({
  items,
  value,
  onValueChange,
  label = "Neighborhoods",
  ...
  isLoading = false,
}: ComboboxNeighborhoodsProps) {
  const id = React.useId();
  const containerRef = React.useRef<HTMLDivElement | null>(null);
  //item.name is a string. Check if can be converted to a number , and sort by number if possible, otherwise sort alphabetically
  ...
  return (
    <Combobox.Root
      items={sortedItems}
      value={
        value ? sortedItems.filter((item) => value.includes(item.name)) : []
      }
    ...
    >
      <div className="flex flex-col gap-1 text-sm font-medium">
        ...
        >
          <Combobox.Value>
            {(selectedValue: BoundaryData[]) => (
              <React.Fragment>
                {selectedValue.map((neighborhood) => (
                  <Combobox.Chip
