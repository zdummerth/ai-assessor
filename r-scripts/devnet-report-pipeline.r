...
result <- call_api(
  endpoint = "/addresses_lookup_standardized", 
  range_start = 0, range_end = 50000,
  params = list(
    select = "address_id",
    tigerline_id = "is.null"
  )
)

weird = "256032f8948fb7e069d66e3b0c60077e"

x = addr_re %>% 
  filter(parcel_id == 29172902)

# This is for reference of how address ids are created with hash algo md5
#addr_re <- test_combine_most_recent_files("supabase-report-transformations/data/site_address_export_re") %>% 
#  convert_parcel_number(property_key) %>% 
#  rename(
#    parcel_id = property_key
#  ) %>%
#  select(-city_state) %>% 
#  mutate(city = "SAINT LOUIS", state = "MO") %>% 
#  mutate(
#    across(c(house_number, prefix_directional, street_name, street_suffix, city, state, zip_code), 
#          ~ replace_na(as.character(.), "")),
#    street_address = str_trim(str_squish(paste(house_number, prefix_directional, street_name, street_suffix))),
#    full_address = str_trim(str_squish(paste(street_address, city, state, zip_code))),
#    address_id = map_chr(street_address, ~ digest::digest(.x, algo = "md5"))
#  )

addresses_lookup <- read_csv(
  "census-geocoded-addresses/with_prefix/geocoded_results.csv"
) %>%
  select(address_id, tigerline_id, tigerline_id_side, raw_address = output_address, lon, lat) %>%
  distinct() %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326)

less_geom <- addresses_lookup %>%
  mutate(geom = st_as_text(geometry)) %>%
  st_drop_geometry()
insert_batches(less_geom, 10000, "addresses_lookup_temp")

addresses_with_no_tiger_match = read_csv("census-geocoded-addresses/with_prefix/geocoded_results_no_matches.csv") %>%
  select(address_id, tigerline_id, tigerline_id_side, raw_address = input_address) %>% 
  distinct(address_id, .keep_all = TRUE)

insert_batches(addresses_with_no_tiger_match, 10000, "addresses_lookup_temp")


parcel_info_raw = test_combine_most_recent_files("G:/R-Projects/general/supabase-report-transformations/data/parcel_info_exports")
parcel_info_raw = get_nas_per_column(parcel_info_raw, TRUE) %>% 
  convert_parcel_number(parcel)

parcel_detail = parcel_info_raw %>% 
...
insert_batches(parcel_detail, 15000, '/parcel_detail')

parcel_deeds = parcel_info_raw %>% 
  get_nas_per_column(TRUE) %>% 
  filter(!is.na(deed_date)) %>% 
  mutate(deed_date = mdy(deed_date)) %>% 
  select(
    parcel_id = parcel,
    year = assessor_year,
    deed_book,
...
parcel_addresses_raw <- read_csv(
  ...
  mutate(
    ...
    is_primary = if_else(is_primary == 1, TRUE, FALSE)
  ) %>% select(-location)

parcel_addresses = parcel_addresses_raw %>% 
  ...
  semi_join(current_standard, join_by(address_id)) %>% 
  distinct() %>%
  group_by(parcel_id) %>%
  mutate(
    is_primary = if_else(row_number() == 1, 1L, 0L)
  ) %>%
  ungroup()

insert_batches(parcel_addresses, 10000, '/parcel_addresses')

retired_parcel_addresses = parcel_addresses_raw %>% 
  filter(!is.na(last_year)) %>% 
  select(
    parcel_id,
    address_id,
  ...
  distinct()
insert_batches(retired_parcel_addresses, 10000, '/retired_parcel_addresses')
...
building_name_to_type <- all_bldg %>%
  distinct(BldgName, BldgType) %>%
  filter(!is.na(BldgType) & BldgType > 0)

building_name_to_cat <- all_bldg %>%
...
com_cost_base <- com_ms_cost_raw %>%
  ...
  left_join(building_name_to_cat,           join_by(structure == BldgName)) %>%
  left_join(building_name_to_type,          join_by(structure == BldgName)) %>%
  rename(
    sqft               = units,
    rcn                = repl_cost_new,
